...
export default function EditarArtigoPage() {
  ...
  if (loading) {
    return <div className="flex h-full items-center justify-center p-16"><div className="h-6 w-6 animate-spin rounded-full border-2 border-cobalt border-t-transparent" /></div>;
  }
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-black text-cobalt">Editar Artigo</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6">
        ...
        <div className="rounded-[10px] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex items-start gap-4">
            {imagePreview ? (
              ...
              <div className="flex h-24 w-40 items-center justify-center rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 text-slate-400"><Upload size={20} /></div>
            )}
        ...
        <div className="flex justify-end gap-3 pb-8">
          <Link href="/admin/artigos" className="rounded-xl border border-slate-200 px-6 py-3 text-sm font-bold text-slate-600 transition hover:bg-slate-50">Cancelar</Link>
          <button type="submit" disabled={saving} className="flex items-center gap-2 rounded-xl bg-cobalt px-6 py-3 text-sm font-black text-white transition hover:bg-cobalt disabled:opacity-60">
