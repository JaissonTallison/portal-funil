...
import { filterListings, getActiveListings } from "@/lib/classifieds";
import type { ListingType } from "@/lib/classifieds";
...
type Props = {
  searchParams: Promise<{ categoria?: string; tipo?: string; busca?: string }>;
};
...
export default async function ClassificadosPage({ searchParams }: Props) {
  ...
  const results = filterListings({
    ...
    type: params.tipo as ListingType | undefined,
    search: params.busca,
  });

  const totalActive = getActiveListings().length;
        ...
        <div className="mb-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-3 max-w-xl text-lg text-slate-500">
              ...
              <span className="ml-2 font-bold text-navy">{totalActive} anúncios ativos</span>
            </p>
          </div>
