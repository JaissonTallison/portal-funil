          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_320px]">
            <div className="rounded-[16px] border border-black/5 bg-white p-6 sm:p-10 shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
              ...
              {protocol ? (
                ...
                <>
                  <div className="mb-6 flex items-center gap-3 rounded-xl border border-purple-100 bg-purple-50 px-5 py-3">
                  ...
                  <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                    <div>
                      ...
                      <input value={form.location} onChange={(e) => update("location", e.target.value)} type="text" placeholder="Bairro, zona da cidade ou localidade" className="h-[52px] w-full rounded-xl border border-slate-200 bg-slate-50 px-5 text-sm outline-none placeholder:text-slate-400 focus:border-gold/50 focus:bg-white" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-semibold">Descrição completa *</label>
                      <textarea value={form.description} onChange={(e) => update("description", e.target.value)} rows={8} placeholder="Descreva tudo que sabe sobre o caso. Datas, locais, pessoas envolvidas, como acontece e qual é o impacto..." className="w-full rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm outline-none placeholder:text-slate-400 focus:border-gold/50 focus:bg-white resize-none" />
                    </div>

                    {error && <p className="rounded-xl border border-red-200 bg-red-50 px-5 py-3 text-sm text-red-700">{error}</p>}

                    <button type="submit" disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 py-4 text-sm font-black uppercase tracking-wide text-white transition hover:bg-purple-700 disabled:opacity-60">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={title} className="rounded-[12px] border border-black/5 bg-white p-7 shadow-[0_10px_40px_rgba(15,23,42,0.05)]">
              ...
              <Link href="/denuncias/midia" className="flex items-center justify-between rounded-[12px] border border-black/5 bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5">
                <div>
