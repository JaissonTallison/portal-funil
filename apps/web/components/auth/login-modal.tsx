...
export function LoginModal({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex overflow-y-auto overscroll-contain px-4 py-4">
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />

      <div className="relative m-auto w-full max-w-md overflow-hidden rounded-[14px] border border-black/5 bg-white shadow-[0_40px_120px_rgba(15,23,42,0.25)]">
        {/* HEADER */}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-7 py-7">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-navy px-5 py-3.5 text-sm font-black text-white transition hover:bg-cobalt disabled:opacity-60"
          >
            {loading ? (
